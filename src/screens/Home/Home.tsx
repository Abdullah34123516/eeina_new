...
import { RecipeInputSection } from "./sections/RecipeInputSection/RecipeInputSection";
...
import { Plus, TrendingUp, Clock, Users, Star, ChefHat, Heart, Bookmark, Play, Award, Siren as Fire, Globe, Camera, MessageCircle, Share2, Calendar } from "lucide-react";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-3 order-2 lg:order-1">
                ...
                <div className="space-y-3">
                  ...
                  <Link to="/planner">
                    <Button variant="outline" className="w-full justify-start">
                      ...
                      {language === 'ar' ? 'مخطط الوجبات' : 'Meal Planner'}
                    </Button>
                  </Link>
                  <Link to="/lists">
                    <Button variant="outline" className="w-full justify-start">
            ...
            <Card className="hidden lg:block">
              <CardContent className="p-4 sm:p-6">
                <h3 className="font-bold text-gray-900 mb-4">{language === 'ar' ? 'استيراد الوصفة':'Import Recipe'}</h3>
                <div className="space-y-3">
            
                  <Input
                    placeholder={language === 'ar' ? 'أدخل رابط الوصفة' : 'Enter recipe URL'}
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1">
                              <Button
                                ...
                                className={`w-6 h-6 ${likedRecipes.includes(recipe.id) ? 'text-red-500' : 'text-gray-400 hover:text-red-500'}`}
                              >
                                <Heart className={`w-3 h-3 ${likedRecipes.includes(recipe.id) ? 'fill-current' : ''}`} />
                              </Button>
                              <Button
                                variant="ghost"
                                size="icon"
                                ...
                                className={`w-6 h-6 ${savedRecipes.includes(recipe.id) ? 'text-yellow-500' : 'text-gray-400 hover:text-yellow-500'}`}
                              >
                                <Bookmark className={`w-3 h-3 ${savedRecipes.includes(recipe.id) ? 'fill-current' : ''}`} />
                              </Button>
                            </div>
                          </div>
                        </div>
                ...
                <CardContent className="p-4 sm:p-6">
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                    ...
                    <div className="flex-1">
                      <Link to={getLocalizedPath("/profile")} className="font-semibold text-sm sm:text-base text-black hover:text-[#22ae4b] transition-colors">
                        Adam Ahmed
                      </Link>
                  ...
                  <div className="mb-4 sm:mb-6">
                    <p className="font-semibold text-black text-base sm:text-lg mb-3 sm:mb-4">
                      {language === 'ar' ? 'لقد صنعت هذه المعكرونة المنزلية الرائعة! السر في الأعشاب الطازجة 🌿' : 'Just made this incredible homemade pasta! The secret is in the fresh herbs 🌿'}
                    </p>
                    <Link to={getLocalizedPath("/recipe/1")} className="block rounded-2xl overflow-hidden hover:scale-[1.02] transition-transform">
                      <img
                  ...
                  <div className="flex items-center gap-4 sm:gap-6 mb-4">
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      className={`flex items-center gap-2 ${likedRecipes.includes(1) ? 'text-red-500' : 'text-gray-600 hover:text-red-500'}`}
                    ...
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      className={`ml-auto ${savedRecipes.includes(1) ? 'text-yellow-500' : 'text-gray-600 hover:text-yellow-500'}`}
                  ...
                  {/* Comment Input */}
                  <form onSubmit={handleComment} className="flex items-center gap-3 p-3 rounded-full border border-gray-300 bg-gray-50">
                    ...
                    <Input
                      ...
                      className={`flex-1 border-0 bg-transparent text-gray-700 focus-visible:ring-0 focus-visible:ring-offset-0 px-0 ${isRTL ? 'text-right' : 'text-left'}`}
                    />
                    {newComment.trim() && (
                      <Button type="submit" size="sm" className="bg-[#22ae4b] hover:bg-[#1c9a40] text-white rounded-full px-4">
          ...
          <div className="lg:col-span-3 order-3">
            ...
            <Card className="mb-6">
              <CardContent className="p-4 sm:p-6">
                ...
                >
                  {showAllCreators ? (language === 'ar' ? 'عرض أقل' : 'Show Less') : (language === 'ar' ? 'عرض المزيد من الطهاة' : 'Show More Creators')}
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="mb-6">
              <CardContent className="p-4 sm:p-6">
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h3 className="font-bold text-gray-900">{t.home.trending_now}</h3>
                </div>

                <div className="space-y-3">
                  {[
                    { name: "Asian Fusion", count: "2.4k recipes", trend: "+12%" },
                    { name: "Healthy Bowls", count: "1.8k recipes", trend: "+8%" },
                    { name: "Quick Meals", count: "3.2k recipes", trend: "+15%" },
                    { name: "Desserts", count: "1.5k recipes", trend: "+5%" }
                  ].map((category, index) => (
