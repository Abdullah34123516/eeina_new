...
export const MealPlanner = (): JSX.Element => {
  ...
  const formatMealTypeName = (mealType: string) => {
    if (mealType.includes('-')) {
      ...
      return mealType.split('-').map(part => {
  ...
  const addCustomMealType = () => {
    if (customMealName.trim() && customMealTime.trim()) {
      const customKey = `${customMealTime.toLowerCase().replace(/\s+/g, '')}-${customMealName.toLowerCase().replace(/\s+/g, '')}`;
  ...
  const MealCard = ({ meal, mealType, onAdd, onRemove }: { meal?: any, mealType: string, onAdd: () => void, onRemove?: () => void }) => {
    const mealIcons = {
      breakfast: Coffee,
      lunch: Sun,
      dinner: Moon,
          ...
          {meals.length === 0 ? (
            ...
            <div className="space-y-3">
              {meals.map((meal, index) => (
                <div key={`${meal.id}-${index}`} className="flex gap-2 sm:gap-3 p-2 bg-gray-50 rounded-lg group">
                  <img
                    src={meal.image}
                    alt={meal.name}
                  ...
                  <div className="flex-1">
                    <h5 className="font-medium text-gray-900 mb-1 text-sm sm:text-base line-clamp-2">{meal.name}</h5>
    ...
    <Card className="group hover:shadow-md transition-all duration-300 cursor-pointer">
      <CardContent className="p-0">
        <div className="relative">
          ...
          />
          <Badge className="absolute top-2 left-2 bg-[#22ae4b] text-white text-xs">
            {recipe.category}
          </Badge>
            ...
            <div className="p-6 border-b border-gray-200 bg-white">
              ...
              {selectedMealType && (
                ...
                <Button
                  ...
                  className={activeTab === 'trending' ? 'bg-[#22ae4b] hover:bg-[#1c9a40]' : 'bg-white hover:bg-gray-50'}
                >
                ...
                <Button
                  ...
                  className={activeTab === 'saved' ? 'bg-[#22ae4b] hover:bg-[#1c9a40]' : 'bg-white hover:bg-gray-50'}
                >
                ...
                <Button
                  ...
                  className={activeTab === 'search' ? 'bg-[#22ae4b] hover:bg-[#1c9a40]' : 'bg-white hover:bg-gray-50'}
                >
  ...
  const NutritionProgress = ({ label, current, goal, unit, color }: {
    label: string;
    current: number;
    goal: number;
    unit: string;
    ...
    return (
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm text-gray-600">{current}/{goal}{unit}</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-8">
            <div className="mb-4 sm:mb-6">
              ...
              <p className="text-sm sm:text-base text-gray-600">{language === 'ar' ? 'خطط وجباتك لهذا اليوم' : 'Plan your meals for the day'}</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              ...
              <EmptyMealCard 
                onAdd={() => setShowCustomMealModal(true)}
              />
            </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                  {trendingRecipes.slice(0, 4).map((recipe) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-500">{recipe.calories} cal • {recipe.time}</div>
                    </div>
                  ))}
                </div>
              </CardContent>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm text-gray-600">
                          {Math.round((selectedDayNutrition.calories / nutritionGoals.calories) * 100)}% Complete
                        </span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div
                          className="h-3 rounded-full bg-[#22ae4b] transition-all duration-300"
                          style={{ width: `${Math.min((selectedDayNutrition.calories / nutritionGoals.calories) * 100, 100)}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 sm:gap-4 text-center text-sm mt-6">
                      <div>
                        <div className="bg-red-50 rounded-lg p-3">
                          <div className="text-red-600 font-bold text-lg">{selectedDayNutrition.protein}g</div>
                          ...
                          <div className="w-full bg-red-200 rounded-full h-1 mt-2">
                            <div
                              ...
                              style={{ width: `${Math.min((selectedDayNutrition.protein / nutritionGoals.protein) * 100, 100)}%` }}
                            />
                          </div>
                        </div>
                      </div>
                      <div>
                        <div className="bg-yellow-50 rounded-lg p-3">
                          <div className="text-yellow-600 font-bold text-lg">{selectedDayNutrition.carbs}g</div>
                          ...
                          <div className="w-full bg-yellow-200 rounded-full h-1 mt-2">
                            <div
                              ...
                              style={{ width: `${Math.min((selectedDayNutrition.carbs / nutritionGoals.carbs) * 100, 100)}%` }}
                            />
                          </div>
                        </div>
                      </div>
                      <div>
                        <div className="bg-blue-50 rounded-lg p-3">
                          <div className="text-blue-600 font-bold text-lg">{selectedDayNutrition.fat}g</div>
                          <div className="text-blue-500 text-xs font-medium">Fat</div>
                          <div className="w-full bg-blue-200 rounded-full h-1 mt-2">
                            <div
                              className="h-1 rounded-full bg-blue-500 transition-all duration-300"
                              style={{ width: `${Math.min((selectedDayNutrition.fat / nutritionGoals.fat) * 100, 100)}%` }}
                            />
                          </div>
                        </div>
                      </div>
              ...
              <Card>
                <CardContent className="p-6">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-lg font-bold text-gray-900">{language === 'ar' ? 'نصيحة اليوم' : "Today's Tip"}</h3>
                  </div>
                  <p className="text-gray-600 text-sm leading-relaxed">
                    {language === 'ar' ? 'حاول تضمين الخضروات الملونة في وجباتك للحصول على تغذية أفضل وفوائد صحية.' : 'Try to include colorful vegetables in your meals for better nutrition and health benefits.'}
                  </p>
                </CardContent>
              </Card>
            </div>
