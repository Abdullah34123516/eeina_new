...
import { ChevronLeft, ChevronRight, Clock, Users, ChefHat, Star, Plus, Minus, Send } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
            ...
            <div className="relative h-[400px] sm:h-[500px] lg:h-[600px] p-2 sm:p-4">
              ...
              <div className="relative h-[320px] sm:h-[420px] lg:h-[500px] rounded-2xl overflow-hidden bg-white shadow-lg mb-4">
                ...
                <Button
                  ...
                  onClick={() => setCurrentImageIndex(prev => prev === 0 ? recipeImages.length - 1 : prev - 1)}
                >
                ...
                <Button
                  ...
                  onClick={() => setCurrentImageIndex(prev => prev === recipeImages.length - 1 ? 0 : prev + 1)}
                >
          ...
          <div className="lg:col-span-5">
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 mb-4 sm:mb-6">
                {language === 'ar' ? 'بولينتا الفطر البورسيني الكريمية' : 'Creamy Porcini Mushroom Polenta'}
              </h1>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
                <div className="text-center">
                  ...
                  <div className="font-semibold text-gray-900 text-sm sm:text-base">5 min</div>
                  <div className="text-xs sm:text-sm text-gray-500">{t.recipe.cooking_time}</div>
                </div>
                <div className="text-center">
                  <div className="flex items-center justify-center mb-2">
                    <Users className="w-4 h-4 sm:w-5 sm:h-5 text-gray-500 mr-1 sm:mr-2" />
                  </div>
                  <div className="font-semibold text-gray-900 text-sm sm:text-base">{language === 'ar' ? 'لبناني' : 'Lebanese'}</div>
                  <div className="text-xs sm:text-sm text-gray-500">{t.recipe.cuisine}</div>
                </div>
                <div className="text-center">
                  <div className="flex items-center justify-center mb-2">
                    <ChefHat className="w-4 h-4 sm:w-5 sm:h-5 text-gray-500 mr-1 sm:mr-2" />
                  </div>
                  <div className="font-semibold text-gray-900 text-sm sm:text-base">{language === 'ar' ? 'يكفي لـ 4' : 'Serves 4'}</div>
                  <div className="text-xs sm:text-sm text-gray-500">{language === 'ar' ? 'الحصة' : 'Serving'}</div>
                </div>
                <div className="text-center">
                  <div className="flex items-center justify-center mb-2">
                    <Star className="w-4 h-4 sm:w-5 sm:h-5 text-gray-500 mr-1 sm:mr-2" />
                  </div>
                  <div className="font-semibold text-gray-900 text-sm sm:text-base">{t.recipe.beginner}</div>
                  <div className="text-xs sm:text-sm text-gray-500">{language === 'ar' ? 'درجة الصعوبة' : 'Degree of Difficulty'}</div>
                </div>
              </div>
              ...
              <Card className="bg-[#22ae4b] text-white rounded-2xl mb-6 sm:mb-8">
                <CardContent className="p-4 sm:p-6">
                  <h3 className="font-bold text-base sm:text-lg mb-3 sm:mb-4">{t.recipe.nutritional_info}</h3>
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-4 text-center">
                    <div>
                      <div className="text-lg sm:text-xl font-bold">200g</div>
                      <div className="text-xs sm:text-sm opacity-90">{t.recipe.fat}</div>
                    </div>
                    <div>
                      <div className="text-lg sm:text-xl font-bold">900g</div>
                      <div className="text-xs sm:text-sm opacity-90">{t.recipe.protein}</div>
                    </div>
                    <div>
                      <div className="text-lg sm:text-xl font-bold">140g</div>
                      <div className="text-xs sm:text-sm opacity-90">Sugars</div>
                    </div>
                    <div>
                      <div className="text-lg sm:text-xl font-bold">435g</div>
                      <div className="text-xs sm:text-sm opacity-90">{t.recipe.calories}</div>
                    </div>
                    <div>
                      <div className="text-lg sm:text-xl font-bold">24g</div>
                      <div className="text-xs sm:text-sm opacity-90">{t.recipe.carbs}</div>
                    </div>
                    <div>
                      <div className="text-lg sm:text-xl font-bold">137g</div>
                      <div className="text-xs sm:text-sm opacity-90">Trans fat</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <div className="mb-6 sm:mb-8">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3">
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{t.recipe.ingredients}</h3>
                  <div className="flex items-center gap-2 sm:gap-4">
                    <span className="text-sm sm:text-base text-gray-600">{servings} {t.recipe.servings}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {ingredients.slice(0, Math.ceil(ingredients.length / 2)).map((ingredient) => (
                      <div
                        key={ingredient.id}
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {ingredients.slice(Math.ceil(ingredients.length / 2)).map((ingredient) => (
                      <div
                        key={ingredient.id}
                        className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg hover:bg-gray-50 cursor-pointer"
                        onClick={() => toggleIngredient(ingredient.id)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-8">
            <div className="mb-6 sm:mb-8">
              <h3 className="text-lg sm:text-xl font-bold text-[#22ae4b] mb-2">Instructions</h3>
              <h4 className="text-xl sm:text-2xl font-bold text-gray-900 mb-6 sm:mb-8">{language === 'ar' ? 'تعليمات الطبخ' : 'Cooking Instructions'}</h4>
                ...
                {cookingSteps.map((step) => (
                  <div key={step.id} className="flex gap-3 sm:gap-6">
                    ...
                    <div className="flex-1">
                      <h5 className="font-bold text-lg sm:text-xl mb-3 sm:mb-4 text-gray-900">{step.title}</h5>
                      ...
                      {step.hasTimer && (
                        <div className="flex items-center gap-3 sm:gap-6 mb-4 sm:mb-6">
                          <div className="flex items-center gap-2 sm:gap-4">
                            ...
                            <span className="font-bold text-sm sm:text-lg">{step.time}</span>
                            <div className="w-6 h-6 sm:w-8 sm:h-8 border-2 border-gray-300 rounded-lg flex items-center justify-center">
                              <div className="w-3 h-3 sm:w-4 sm:h-4 bg-gray-300 rounded-full" />
                            </div>
                          </div>
                      ...
                      <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        />
                        <span className="text-gray-600 text-sm sm:text-base">{t.recipe.mark_complete}</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {/* Comments Section */}
            <div className="mb-6 sm:mb-8">
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-6 sm:mb-8">{t.recipe.comments}</h3>
                  ...
                  <div key={comment.id} className="flex gap-3 sm:gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 mb-2">
                        <span className="font-bold text-gray-900 text-sm sm:text-base">{comment.user}</span>
                        <span className="text-gray-500 text-xs sm:text-sm">{comment.date}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <span className="text-xs sm:text-sm text-gray-500">5.00 of 5</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-50 rounded-2xl">
                ...
                <Input
                  ...
                  className={`flex-1 border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 text-gray-700 text-sm sm:text-base ${isRTL ? 'text-right' : 'text-left'}`}
                />
              ...
              <Card className="rounded-2xl">
                <CardContent className="p-4 sm:p-6">
                  <div className="flex items-center justify-between mb-4 sm:mb-6">
                    <h3 className="text-lg sm:text-xl font-bold text-gray-900">{language === 'ar' ? 'إجمالي التغذية' : 'Nutrition Totals'}</h3>
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {nutritionData.map((item, index) => (
                      <div key={index} className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-[#22ae4b] text-xs sm:text-sm">{item.value}</span>
                      </div>
                    ))}
                  </div>
